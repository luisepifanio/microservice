...
public class JSONResponseBuilder<T> {
    ...
    public static <R> JSONResponseBuilder builder() {
    ...
    protected Response handle(FailedResponse response) {
        ...
        JSONResponse<T> jsonResponse = new JSONResponse<T>(serviceStatus, null);
    ...
    protected Response handle(ar.com.phosgenos.dto.response.Response<T> response) {
        ...
        Optional<ServiceStatus> serviceStatus = Optional.ofNullable(status);

        JSONResponse<T> jsonResponse = new JSONResponse<T>(
                serviceStatus.orElse(ServiceStatus.builder().status(200).build()),
                response.getData()
        );

        return Response
                .status(jsonResponse.getStatus().getStatus())
                .entity(jsonResponse)
                .build();
    }
}
